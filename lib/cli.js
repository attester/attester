...
var end_process = require('./end-process.js');
...
var run = function () {
    var opt = optimist.usage('Usage: $0 [options] [config.yml|config.json]').boolean(['json-console', 'help',
            'server-only', 'version', 'colors', 'ignore-errors', 'ignore-failures']).string(['phantomjs-path']).describe({
    ...
    var endProcess = function () {
        // In this method, we should close anything which would make
        // node wait for something...
        if (testServer) {
            testServer.close();
        }
    ...
    var logger = new Logger("atjstestrunner");
