...
var run = function () {
    var opt = optimist.usage('Usage: $0 [options] [config.yml|config.json]').boolean(['flash-policy-server',
            'json-console', 'help', 'server-only', 'version', 'colors', 'ignore-errors', 'ignore-failures']).string(['phantomjs-path']).describe({
    ...
    var boolCanExit = false;
    var endProcessWrap = function (success) {
        if (boolCanExit) {
            endProcess(success);
        } else {
            boolCanExit = true;
    ...
    campaign.on('finished', function () {
        writeReports({
            ...
            logger.logInfo('Tests run: %d, Failures: %d, Errors: %d, Skipped: %d', [stats.testCases, stats.failures,
                    stats.errors, stats.tasksIgnored]);
            endProcessWrap(success);
        });

        var mailcfg = configData['email-reports'];
        if (mailcfg && htmlReport.stats.errors > 0) {
            emailReports({
                text : mailcfg['text'],
                from : mailcfg['from'],
                to : mailcfg['to'],
                ...
                if (msg && msg.err) {
                    logger.logError('An error occurred while emailing reports: %s', [JSON.stringify(msg.err)]);
                } else if (msg && msg.success) {
                    logger.logInfo('Email(s) sent');
                    // logger.logInfo('Report emailing: %s', [JSON.stringify(msg.success)]);
                }
                endProcessWrap();
            });
        } else {
