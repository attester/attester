    ...
    if (phantomJSinstances > 0) {
        campaign.on('result-serverAttached', function (event) {
            ...
            var checkPhantomjsSpawnExitCode = function (code) {
                if(code === 127) {
                    logger.logError("Spawn: exited with code 127. PhantomJS executable not found. Make sure to download PhantomJS and add " +
                                    "its folder to your system's PATH, or pass the full path directly to Attester via --phantomjs-path.\n" +
                                    "Used command: '" + path + "'");
                } else if (code === 126) {
