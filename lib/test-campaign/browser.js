...
Browser.prototype.matches = function (slave) {
    ...
    var browserInfo = slave.browserInfo;
    var match = true;
    if (config.browserName != null) {
        match = match && (config.browserName == browserInfo.name);
    }
    if (config.majorVersion != null) {
        match = match && (config.majorVersion == browserInfo.majorVersion);
    }
    if (config.minorVersion != null) {
        match = match && (config.majorVersion == browserInfo.minorVersion);
    }
    return match;
};
