...
var TestCampaign = function (config, parentLogger) {
    ...
    if (config.browsers && config.browsers.length > 0) {
        ...
        this.logger.logInfo("No external browsers expected in the campaign");
    }
    ...
    if (coverage) {
        app.use('/__attester__/coverage/display', coverage.resultsSender('/__attester__/coverage/display', config['coverage-reports']['json-file']));
    }
...
/**
 * Given the physical browser's info, find matching Browser object in the campaign.
 * @param {Object} browserInfo coming from Slave's this.browserInfo
 * @return {Browser}
 */
TestCampaign.prototype.findCampaignBrowser = function (browserInfo) {
    if (!browserInfo) {
        return null;
    }
    for (var i = 0; i < this.browsers.length; i++) {
        var browser = this.browsers[i];
        if (browserMatch(browser.config, browserInfo)) {
            return browser;
        }
    }
    return null;
