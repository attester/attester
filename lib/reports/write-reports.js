...
var writeReports = function (config, results, callback) {
    ...
    var writeHtml = function (files, html) {
        if (html && files.length > 0) {
            writeMultipleFiles(files, html);
        }
    };
    ...
    var htmlResults = results.htmlReport;
    var coverageResults = results.coverage;

    writeJson(testConfig['json-file'], testResults.report);
    console.log(JSON.stringify("html::" + htmlResults.report));
    writeHtml(testConfig['html-file'], htmlResults.getContent());
