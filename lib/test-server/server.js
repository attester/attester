...
var statusPage = function (req, res, next) {
    ...
    var slaves = testServer.slaves, l = slaves.length;
    content.push('<h2>Connected browsers</h2>');
    if (l > 0) {
        content.push('<table><tr><th style="width:150px;">Browser</th><th style="width:300px;">User agent</th><th style="width:150px;">Address</th><th>Test</th></tr>');
        for (var i = 0; i < l; i++) {
            var curSlave = slaves[i];
            content.push('<tr>');
            content.push('<td>', curSlave.displayName, '</td>');
            content.push('<td>', curSlave.userAgent, '</td>');
            content.push('<td>', curSlave.address, ':', curSlave.port, '</td>');
            if (curSlave.currentTask) {
                content.push('<td>', curSlave.currentTask.test.name, '</td>');
            } else {
                content.push('<td>(idle)</td>');
            }
            content.push('</tr>');
        }
        content.push('</table>');
...
var TestServer = function (config, logger) {
    ...
    this.socketio.disable('flash policy server'); // the flash policy server must be disabled before enabling the
    // flashsocket transport
