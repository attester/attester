...
// middlewares
var cacheControl = require("../middlewares/cacheControl");
var index = require("../middlewares/index");
var template = require("../middlewares/template");

var nonCacheable = /__attester__/;
var cacheable = /(iframe|run)\.js$/;
...
var TestServer = function (config, logger) {
    ...
    app.use(cacheControl({
        logger: this.logger,
        cacheEnabled: config.predictableUrls,
        cacheWhiteList: cacheable,
        cacheBlackList: nonCacheable
    }));
    ...
    this.socketioLogger = new SocketIOLogger('socketio', this.logger);
    this.socketio = socketio.listen(this.server, {
        logger: this.socketioLogger
    });
