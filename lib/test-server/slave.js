...
var getBrowserDisplayName = function (browserInfo) {
    var res = [browserInfo.name];
    if (!isNaN(browserInfo.majorVersion)) {
        res.push(' ', browserInfo.majorVersion);
        if (!isNaN(browserInfo.minorVersion)) {
            res.push('.', browserInfo.minorVersion);
            if (!isNaN(browserInfo.revision)) {
                res.push('.', browserInfo.revision);
            }
        }
    }
    return res.join('');
...
var Slave = function (socket, data) {
    ...
    this.displayName = getBrowserDisplayName(this.browserInfo);
