...
page.onInitialized = function (arg1) {
    page.evaluate(function () {
        window.atjstestrunnerConfig = {
            globalErrors : true,
            onDisconnect : function () {
                console.log("PHANTOMJS-ATJSTESTRUNNER-DISCONNECT");
            }
        };
    });
};
...
page.onConsoleMessage = function (msg) {
    if (msg == "PHANTOMJS-ATJSTESTRUNNER-DISCONNECT") {
        if (autoExit) {
...
page.onError = function (error, stack) {
    ...
    page.evaluate("function(){window.atjstestrunner.testError(" + JSON.stringify(error) + ");}");
};
...
if (autoExit) {
    var checkFunction = (function () {
        if (!window.atjstestrunner || !window.atjstestrunner.connected) {
            console.log("PHANTOMJS-ATJSTESTRUNNER-DISCONNECT");
        }
    }).toString();
