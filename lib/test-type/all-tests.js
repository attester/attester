...
var testTypePrototypes = {
    'aria-templates' : require('./aria-templates/at-test-type.js')
};

var AllTests = function (campaign, config) {
    ...
    for (var type in config) {
        if (testTypePrototypes.hasOwnProperty(type)) {
            ...
            // TODO: report the test type as not being supported
        }
    }
...
var buildTasksForTest = function (testType, test) {
    ...
    if (!subTests && !test.url && !testResults) {
        ...
        testResults = [{
                    ...
                    name : test.name,
                    error : {
                        message : "This test does not have any url (this is probably an issue from the runner from "
                                + testType.name + " test runner)."
                    }
                }, {
                    event : "taskFinished"
                }];
    ...
    if (subTests) {
        ...
        if (browsersCount > 1) {
            var tasks = [];
            for (var i = 0; i < browsersCount; i++) {
                tasks[i] = buildTask.call(this, testType, test, browsers[i]);
            }
            return {
                name : test.name,
                subTasks : tasks
...
AllTests.prototype.init = function (callback) {
    var expectedCallbacks = this.testTypesCount;
