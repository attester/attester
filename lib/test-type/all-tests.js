...
var AllTests = function (campaign, config, parentLogger) {
    ...
    for (var type in config) {
        if (testTypePrototypes.hasOwnProperty(type)) {
            ...
            curTestType.setBaseURL(campaign.baseURL);
...
var buildTasksForTest = function (testType, test) {
    ...
    if (test.url) {
        // Prefix the base URL
        test.url = url.normalize(testType.baseURL, testType.type, test.url);
    }
    if (testResults) {
...
AllTests.prototype.init = function (callback) {
    ...
    var endFunction = function (testType) {
        ...
        if (testType.debug) {
            this.debugUrls.push({
                name: testType.name,
                url: url.normalize(testType.baseURL, testType.type, testType.debug)
            });
        }
