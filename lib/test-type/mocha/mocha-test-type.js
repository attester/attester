...
var staticFile = require('../../middlewares/staticFile');
...
var MochaTestType = function (campaign, inputConfig) {
    ...
    this.use("lib", connect["static"](path.dirname(require.resolve("mocha"))));
    this.use("client", connect["static"](path.join(__dirname, "client")));
    this.use("tests", connect["static"](path.join(__dirname, path.relative(__dirname, testConfig.files.rootDirectory))));
    ...
    if (/^expect([\.\-]?js)?$/.test(assertion)) {
        assertionDescription.src = this.type + "/assertion/expect.js";
        ...
        assertionDescription.src = this.type + "/assertion/chai.js";
        this.use(staticFile.bind({
